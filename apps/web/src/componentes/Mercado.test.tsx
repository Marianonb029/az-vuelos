...
describe("Mercado", () => {
  it("pide el mercado con la ventana elegida, agrupa por aeropuerto de salida y muestra las seis variables y la antigüedad", async () => {
    ...
    await waitFor(() => expect(screen.getByTestId("cobertura").textContent).toContain("Precios actualizados el 2026-09-17: 1 rutas, salidas desde ASU; llegadas a MAD"));
    ...
    expect(screen.getByTestId("cobertura-grupos").textContent).toContain("1. América del Norte+América del Sur → Europa: 120 rutas, 3000 precios, 45 de 1115 aeropuertos de salida recorridos");
    ...
    await waitFor(() => expect(screen.getByTestId("calendario-nota").textContent).toContain("2 días con precio en este mes (en verde, con el más barato en USD) · 2 en total, entre el 19/01/2027 y el 20/01/2027"));
    ...
    expect(cabeceras[0]).toContain("1. Desde ASU (Asunción) — el aeropuerto que pediste · 2 opciones desde USD 630");
    expect(cabeceras[1]).toContain("Desde IGU (Foz do Iguaçu) — a 300 km de ASU; llegar hasta ahí lo pagás aparte · 1 opciones desde USD 300");
    ...
    expect(filas[0]).toContain("2GOLlow cost, TAP"); // GOL con distintivo low cost (cobertura.aerolineasBajoCosto)
    ...
    expect(screen.getByTestId("nota-dataset").textContent).toMatch(/1 actualizaciones guardadas, 1\.?590 precios vigentes, 900 para estos aeropuertos/);
    ...
    expect(screen.getByTestId("dias-frescos").textContent).toContain("Se saltean 1 de los 15 días");
    expect((screen.getByRole("button", { name: "Buscar en Aviasales los 14 días que hacen falta (12/01/2027 a 26/01/2027, ~11 min) y traer los precios" }) as HTMLButtonElement).disabled).toBe(false);
    expect((screen.getByRole("button", { name: "Actualizar esta ruta ahora" }) as HTMLButtonElement).disabled).toBe(false);
  }, 20_000);
});
