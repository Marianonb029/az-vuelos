...
const Fila = ({ r, nombre }: { r: RutaPosible; nombre: (iata: string) => string }) => {
  ...
  return (
    <tr className={`border-b border-slate-100 ${r.conservada ? "" : "text-slate-400"}`} data-testid="fila-posible">
      ...
      <td className={`${celda} whitespace-nowrap`}>{r.tarifasMercado.every((n) => n > 0) ? `sí: ${r.tarifasMercado.join(" + ")} tarifas` : r.tarifasMercado.some((n) => n > 0) ? `parcial: ${r.tarifasMercado.join(" + ")}` : "no"}</td>
    </tr>
  );
};
...
export const Combinaciones = ({ aeropuertos }: Props) => {
  ...
  const pasa = (r: RutaPosible) => f === "" || r.itinerario.some((i) => i.includes(f)) || [...r.aerolineas, ...r.aerolineasPrevio].some((a) => a === f || nombre(a).toUpperCase().includes(f)) || ciudad(r.destino).toUpperCase().includes(f);
  const rutas = (resultado?.rutas ?? []).filter(pasa);
    ...
    <div className="grid gap-6">
      <form onSubmit={(e) => void buscar(e)} noValidate className="grid gap-5">
        <p className="text-xs text-slate-600">
          Todas las rutas que el grafo de aerolíneas permite (rutas vigentes por número de vuelo, VRS) desde el origen y sus alternativos, en un boleto o en dos por un hub. Sin fecha ni precio: es para buscar alternativas a mano cuando el mercado no alcanza; "en el mercado" dice si ese par ya tiene tarifas bajadas.
                      ...
                      {abiertos.has(k) && (
                        <div className="overflow-x-auto pl-4">
                          <table className="w-full text-sm">
                            ...
                            <tbody>
                              {deDestino.map((r) => (
                                <Fila key={`${r.itinerario.join("")}-${r.aerolineas.join("")}-${r.aerolineasPrevio.join("")}`} r={r} nombre={nombre} />
                              ))}
                            </tbody>
                          </table>
                        </div>
