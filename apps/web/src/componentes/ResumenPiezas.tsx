  ...
  return (
    <div className={`grid gap-1 rounded-lg border p-3 ${destacada ? "border-emerald-300 bg-emerald-50" : "border-slate-200 bg-white"}`} data-testid="opcion">
      ...
      <p className="text-xs text-slate-600">
        {fechaCorta(c.fechaIda)} · {horas(c.duracionTotalMin)} · {c.escalas === 0 ? "directo" : `${c.escalas} escala${c.escalas === 1 ? "" : "s"}`} · {c.aerolineas.map(nombre).join(", ")} · valija {c.equipajeBodega === null ? "no se sabe" : c.equipajeBodega ? "incluida" : "no incluida"}
...
export const conclusiones = (m: ResultadoMercado, nombre: (iata: string) => string): string[] => {
  ...
  if (minPedido !== null && barata.trasladoOrigenKm > 0 && minPedido > barata.totalUsd)
    fuera.push(`Salir de ${barata.origen} en vez de ${m.origen} ahorra USD ${(minPedido - barata.totalUsd).toLocaleString("es")} (${Math.round(((minPedido - barata.totalUsd) / minPedido) * 100)} %), pero está a ${barata.trasladoOrigenKm.toLocaleString("es")} km y llegar hasta ahí lo pagás aparte.`);
  else if (minPedido !== null) fuera.push(`Lo más barato sale del aeropuerto que pediste (${m.origen}): no hace falta viajar hasta otro.`);
  ...
  } else fuera.push(`Ninguna opción de estos días es directa: la de menos escalas tiene ${Math.min(...lista.map((c) => c.escalas))}.`);
  ...
  if (mejorDia && peorDia && porDia.length > 1 && peorDia.min > mejorDia.min)
    fuera.push(`De los días que estás mirando, el más barato es ${fechaCorta(mejorDia.f)} (USD ${mejorDia.min.toLocaleString("es")}) y el más caro ${fechaCorta(peorDia.f)} (USD ${peorDia.min.toLocaleString("es")}): mover la salida cambia hasta USD ${(peorDia.min - mejorDia.min).toLocaleString("es")}.`);
  ...
  if (conBodega.length > 0) {
    ...
    fuera.push(`Con valija despachada incluida, lo más barato es USD ${minBodega.toLocaleString("es")}${minBodega > barata.totalUsd ? `: USD ${(minBodega - barata.totalUsd).toLocaleString("es")} más que la opción más barata, que va sólo con equipaje de mano` : ""}.`);
  } else fuera.push(`Ninguna opción de estos días incluye valija despachada: si viajás con valija, contá ese cargo aparte.`);
  ...
  if (dosBoletos.length > 0) {
    ...
    if (minUno !== null && minDos < minUno) fuera.push(`Comprar dos pasajes por separado ahorra USD ${(minUno - minDos).toLocaleString("es")}, pero si el primer vuelo se atrasa perdés el segundo: al ser pasajes distintos, la aerolínea no te cubre la conexión.`);
  }
  const vendedoras = [...new Set(lista.flatMap((c) => c.aerolineas))];
  if (vendedoras.length === 1 && vendedoras[0]) fuera.push(`Todo lo que hay estos días lo vende una sola aerolínea (${nombre(vendedoras[0])}): sin competencia, el precio no tiene con qué bajar.`);
  return fuera;
};
