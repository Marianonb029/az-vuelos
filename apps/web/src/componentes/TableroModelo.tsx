...
// Tablero del modelo sin precios (la parte de abajo de la pestaña): resumen de la última priorización, qué se
// armó y descartó, y por dónde conviene empezar a buscar. Se rehace en cada "Priorizar rutas"; no guarda nada.
export const TableroModelo = ({ resultado }: { resultado: ResultadoRutas | null }) => {
  if (!resultado) return <p className="text-sm text-slate-600">Todavía no hay una priorización del modelo: está en Rutas, abajo, en "Modelo sin precios"; sirve para ver qué pares baja pnpm precios.</p>;
