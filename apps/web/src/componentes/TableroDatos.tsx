...
const GLOSARIO: { termino: string; significado: string; sale: string }[] = [
  ...
  { termino: "Aerolínea de un boleto", significado: "La API devuelve una sola aerolínea por boleto: la que lo vende. Si el boleto tiene escalas, los tramos pueden estar operados por otra (un boleto de Gol con el último tramo de TAP aparece como 'Gol'). Por eso la columna 'aerolíneas' cuenta vendedoras, no operadoras, y lo dice debajo.", sale: "Precios cacheados (Travelpayouts)" },
  { termino: "Equipaje (probable)", significado: "No viene como dato: se deduce de la clave de tarifa del enlace (H = mano, L = bodega), que la API no documenta. Por eso se muestra subrayado y marcado como probable. Confirmalo en la aerolínea antes de comprar; el criterio 3 del orden usa esta deducción.", sale: "Equipaje de mano y de bodega" },
  { termino: "Seguir un par", significado: "Le pide a la bajada nocturna que vuelva a bajar ese par (y su vuelta, si lo pedís) todas las noches, una vez por día. Es lo único que la app guarda por decisión tuya —origen, destino, direcciones y desde cuándo— y se puede deshacer. Sirve para que se arme el historial: sin dos bajadas no hay con qué decir si el precio sube o baja.", sale: "Corridas" },
  { termino: "Tus pares seguidos (panel)", significado: "Arriba de Explorar precios: a cuánto está hoy cada par que seguís, cuánto se movió desde la primera bajada, cuándo se bajó por última vez y qué dice su señal. Si seguís también la vuelta, muestra el total de los dos boletos en sus mejores días. Sin este panel el historial se junta todas las noches y nadie lo mira.", sale: "Corridas" },
  { termino: "Se saltean N días (búsqueda en vivo)", significado: "La búsqueda en vivo no repite los días cuyo precio todavía no venció: 'vencido' es la misma cadencia que usa cada fila (diaria a menos de 14 días del viaje, cada 3 días hasta 60, semanal más lejos). Sólo busca los días sin precio o con precio viejo, y dice cuántos minutos de ventana abierta te ahorra.", sale: "Antigüedad y desvío estimado de cada tarifa" },
  { termino: "Medir cuánto tarda en publicarse", significado: "Hace una sola búsqueda en Aviasales y sondea el cache cada 10 segundos hasta 5 minutos: devuelve el tiempo real que tardó en publicarse. Los 45 s que la app espera entre búsquedas son un supuesto; con dos o tres mediciones ese número se cambia en config con un dato. No cambia nada solo: informa.", sale: "Precios cacheados (Travelpayouts)" },
  { termino: "Comprobación a mano", significado: "`pnpm comprobar` elige tarifas cacheadas de distintas antigüedades y muestra sus enlaces; abrís uno, mirás qué precio tiene hoy en Aviasales y lo anotás con `pnpm comprobar ORIGEN DESTINO FECHA PRECIO` (o 'no' si ya no está). Es la única medida del desvío contra la realidad: el resto se mide entre corridas o se supone. El resultado aparece en la ficha de abajo.", sale: "Comprobación a mano (cacheado contra lo que muestra Aviasales)" },
  { termino: "Orden de la bajada nocturna", significado: "El modelo ordena los destinos de cada origen por lo que prometen —aerolíneas distintas en el tramo, frecuencia, si lo vuela una low cost, si el destino es grande— y el presupuesto se gasta de arriba hacia abajo. Nada se descarta: lo que queda último se baja igual si sobra. Medido sobre el dataset: en los primeros 3 pedidos desde GRU trae 722 tarifas contra 141 con el orden anterior.", sale: "Rutas vigentes (competencia por tramo)" },
  { termino: "Ida y vuelta", significado: "La suma de dos boletos de ida: el de ida el día que salís y el de vuelta el día que volvés, con la estadía que elijas. No son tarifas de ida y vuelta: un boleto de ida y vuelta de la misma aerolínea suele costar menos, así que el total es un techo. Necesita que el sentido inverso también esté bajado.", sale: "Precios cacheados (Travelpayouts)" },
  { termino: "Días sin precio (en blanco)", significado: "Días del calendario en los que el cache no tiene ninguna tarifa: nadie buscó ese día en Aviasales todavía. No significa que no haya vuelos. El botón de cada mes los busca en vivo uno por uno y los trae al sistema.", sale: "Precios cacheados (Travelpayouts)" },
  { termino: "Qué conviene buscar a mano", significado: "En Combinaciones, los boletos que ninguna tarifa cubre, ordenados por lo que aportarían: primero los que tienen ruta directa y más vuelos por semana (más chance de que haya tarifa cuando se busque). El botón los carga en la búsqueda múltiple de Rutas.", sale: "Competencia: aerolíneas por tramo" },
  ...
  { termino: "Bajada por continentes", significado: "La app sirve cualquier par del mundo: el que pidas y no esté cacheado se baja en el momento (Actualizar este par) o se sigue para que entre en la corrida de las 03:00. Los grupos son sólo el ORDEN en que se precarga el cache, y al terminarlos el barrido sigue por todos los aeropuertos del mundo con servicio regular. pnpm precios recorre los aeropuertos de salida de cada grupo de prioridad (1. Sudamérica → Europa, 2. Norteamérica → Europa, 3. Europa → América, 4. Europa → Asia, 5. América → Asia; Rusia excluida), los de más salidas primero: un pedido sin destino descubre a qué destinos hay cache y un pedido por par trae el mínimo de cada fecha. Con presupuesto por corrida; la siguiente sigue donde quedó.", sale: "Precios cacheados (Travelpayouts)" },
