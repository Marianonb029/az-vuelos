...
- `apps/api` — Node 24 + Fastify 5. Cálculo sobre datasets, sin base de datos ni registros de uso: `GET /rutas`, `GET /espacio*`, `GET /datos`. Lee de `data/local/` lo que dejan los scripts (tendencias, corroboración). Refresco automático diario de fuentes vencidas.
...
- `scripts` — catálogos (OurAirports, OpenTravelData, VRS), eventos (Wikidata), corroboración de rutas con Wikipedia y lectura de tendencia de Google Flights (Playwright, sólo ese script).
