...
Muestra **lo que el mercado tiene** para llegar de un origen a un destino: las tarifas cacheadas de la API de datos de Travelpayouts (lo que otros usuarios de Aviasales encontraron), en uno o dos boletos encadenados, sin límite de transbordos, ordenadas por aeropuerto de salida, precio, equipaje de bodega, horas totales, escalas y aerolíneas (Fase 15), con la antigüedad y el desvío estimado de cada tarifa. Sirve **cualquier par del mundo**: el que se pida y no esté cacheado se baja en el momento desde la app o se sigue para que entre en la corrida nocturna. La precarga del cache es **por continentes** y en el orden del dueño (Fase 16: Sudamérica → Europa, Norteamérica → Europa, Europa → América, Europa → Asia, América → Asia; Rusia excluida) y después **sigue por el resto del mundo** (Fase 26: todos los aeropuertos con servicio regular, los de más salidas primero): los grupos son el orden, no el límite. Se descubre desde cada aeropuerto de salida a qué destinos hay cache; el destino de una búsqueda puede ser un aeropuerto o un continente, y el calendario habilita sólo los días con tarifas para ese par. Detrás, el **modelo sin precios** (Fases 1–14) sigue eligiendo los pares de un par concreto (`pnpm precios ORIGEN DESTINO`). El brief original (`docs/BRIEF.md`) pedía leer precios en sitios oficiales; la Fase 9 lo reemplazó por el modelo y la Fase 15 puso los precios ciertos de la API como salida. `docs/DECISIONES.md` manda sobre el brief.
