...
- `apps/api` — Node 24 + Fastify 5. Cálculo sobre datasets, sin base de datos ni registros de uso: `GET /mercado` (Fase 15), `POST /mercado/actualizar` y `GET /mercado/sonda` (Fase 18, con `TRAVELPAYOUTS_TOKEN` en el entorno; `TRAVELPAYOUTS_MARKER` opcional para los enlaces), `GET /rutas-posibles` (Fase 17), `GET /rutas` (modelo), `GET /espacio*`, `GET /datos`. Lee de `data/local/` lo que dejan los scripts (tendencias, corroboración, precios cacheados). Refresco automático diario de fuentes vencidas.
