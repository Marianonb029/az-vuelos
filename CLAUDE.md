...
- `apps/web` — React 18 + Vite + Tailwind v4. Cinco pestañas: Buscar (el panorama del par sin fecha: mapa de calor del año, mejor mes, a qué ciudad y desde qué aeropuerto sale más barato, Fase 21), Combinaciones (todas las rutas del grafo, sin fecha ni precio, Fase 17), Rutas (el mercado: llega exactamente al aeropuerto elegido o a un continente), Tablero (métricas de la última búsqueda, sin registro) y Datos (glosario y ficha de cada dato).
- `apps/api` — Node 24 + Fastify 5. Cálculo sobre datasets, sin base de datos ni registros de uso: `GET /mercado` (Fase 15), `GET /mercado/panorama` (Fase 21), `POST /mercado/actualizar` y `GET /mercado/sonda` (Fase 18, con `TRAVELPAYOUTS_TOKEN` en el entorno; `TRAVELPAYOUTS_MARKER` opcional para los enlaces), `GET /rutas-posibles` (Fase 17), `GET /rutas` (modelo), `GET /espacio*`, `GET /datos`. Lee de `data/local/` lo que dejan los scripts (tendencias, corroboración, precios cacheados). Refresco automático diario de fuentes vencidas.
