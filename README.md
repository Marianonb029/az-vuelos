...
Dado un origen, un destino y una fecha de ida —**cualquier par del mundo**—, AZ Vuelos muestra **lo que el mercado tiene** para llegar: las tarifas que otros usuarios de Aviasales encontraron (API de datos de Travelpayouts, gratuita), en un boleto o en dos encadenados donde termina el primero, sin límite de transbordos, con el orden del dueño:
...
|---|---|
| **Explorar precios** | *¿Cuándo y a dónde me conviene ir?* Origen y destino (aeropuerto o continente), **sin fecha** → el panorama de todo el horizonte del cache: lo más barato con su día y ciudad, el precio típico de un día, el mejor mes y el mejor aeropuerto de salida; un mapa de calor de un año (una celda por día, coloreada por cuantiles de ese par; gris = sin tarifas cacheadas, que es falta de búsquedas, no de vuelos), barras por mes (mínimo contra típico), a qué ciudad del continente se llega más barato, desde qué aeropuerto sale más barato (con el traslado dicho aparte) y las 12 más baratas, una por par salida → llegada. Un clic en cualquier día, mes, ciudad o salida abre Rutas con ese par y ese día. Incluye la curva de anticipación (con cuántos días antes estuvo más barato ese par), el total de **ida y vuelta** para la estadía que elijas y, en cada mes, un botón para buscar en vivo los días que todavía no tienen precio. |
| **Rutas** | *¿Qué hay para este día?* Origen, destino (aeropuerto o continente), fecha —un calendario que habilita sólo los días con tarifas para ese par, con el mínimo de cada día— y ventana (ese día / ±3 / ±7 / ±15) → combinaciones del mercado agrupadas por aeropuerto de salida, con boletos (itinerario, aerolínea, vuelo, horario local, equipaje, agencia, enlace a Aviasales), precio, equipaje, horas totales, escalas, aerolíneas, día de salida y antigüedad con desvío estimado. La llegada es exactamente el aeropuerto elegido (o, con continente, cualquiera con tarifas). |
| **Resumen de ruta** | *¿Qué me conviene de lo que encontré?* Las conclusiones de la última búsqueda de Rutas (no guarda registro): cuatro opciones comparables —la más barata, la más rápida, la de menos escalas y la más equilibrada— cada una con lo que se resigna frente a la más barata; conclusiones en frases con la cuenta a la vista (traslado contra ahorro, qué cuesta la hora ahorrada, el día más barato de la ventana, bodega); la señal **comprar / esperar / volvé a mirar** con la curva de anticipación y el historial del par; qué tan confiable es lo que se ve; y el detalle plegado (escalas, dónde está lo barato, días y estado del dataset). |
| **Combinaciones** | *¿Qué rutas existen, aunque no tengan precio?* Arriba, **qué conviene buscar a mano**: los boletos que ninguna tarifa cubre, ordenados por lo que aportarían, con un botón que los carga en la búsqueda múltiple. Abajo, la lista completa: origen y destino (aeropuerto o continente) → todas las rutas que el grafo de aerolíneas permite desde el origen y sus alternativos, en un boleto o dos por un hub, sin fecha ni precio: itinerario, quién vende cada boleto, quién opera cada tramo, km, frecuencia y si el par ya tiene tarifas en el mercado. Plegado por salida y destino, con filtro. Para buscar alternativas a mano. |
| **Datos** | *¿De dónde sale cada número?* Para qué sirve cada pestaña (pregunta que contesta, objetivo y qué no hace), el glosario de cada término y la ficha de cada dato: fuente, última actualización, exactitud (exacta / vigente / aproximada / supuesto), cadencia de refresco y si venció. |
...
| `pnpm precios [pedidos]` | Precarga del cache (Travelpayouts, token gratuito en `TRAVELPAYOUTS_TOKEN`). **Primero** los pares seguidos (30 % del presupuesto), **después** los corredores de `bajada.grupos` (Sudamérica → Europa, Norteamérica → Europa, Europa → América, Europa → Asia, América → Asia; Rusia excluida), por cada aeropuerto de salida descubre a qué destinos hay cache y baja un pedido por par (el mínimo de cada fecha de todo el horizonte). y **al final el resto del mundo** (todos los aeropuertos con servicio regular, los de más salidas primero): los grupos son el orden, no el límite. Dentro de cada origen, el modelo ordena los destinos por competencia, frecuencia, low cost y tamaño. Hasta 1.500 pedidos por corrida (~25 min); la siguiente sigue donde quedó. Se acumula sin borrar corridas anteriores |
| `pnpm precios ASU MAD` | Lo mismo para los pares de boletos que el modelo propone para un par concreto (~1–2 min) |
| `scripts\az-vuelos.cmd` | Deja la app lista: levanta lo que falte y abre el navegador; con `/sin-navegador` sólo levanta. Es lo que corre el acceso directo "AZ Vuelos" del escritorio |
| `scripts\az-api.cmd` · `scripts\az-web.cmd` | Cada servidor en primer plano con su registro (`data/local/api.log`, `web.log`). Son las acciones de las tareas de Windows **"AZ Vuelos - api"** y **"AZ Vuelos - web"**, que arrancan al iniciar sesión y quedan en estado "En ejecución" mientras el servidor vive (antes una sola tarea lanzaba los dos con `start` y Windows se llevaba el árbol de procesos por delante) |
| `scripts\az-reiniciar.cmd` | Reinicia las dos tareas. Hace falta después de cambiar código del API (la web recarga sola) |
