...
## Fase 15 (17/09/2026) — el mercado: los precios ciertos de la API pasan a ser la salida; el modelo elige qué bajar

Pedido del dueño: cambiar los objetivos con la misma estructura. La información ya no es la del modelo sino la de la API: traer todas las rutas de vuelo, quitar el límite de transbordos y ordenar (1) por aeropuerto de salida, (2) de menor a mayor precio, (3) sin equipaje de bodega antes que con, (4) de menos a más horas totales, (5) de menos a más transbordos, (6) de menos a más aerolíneas distintas. Objetivo: identificar los vuelos baratos del mercado con precios ciertos. Definir cada cuánto actualizar sin borrar los registros anteriores, etiquetar los días transcurridos desde que se tomó cada precio con un desvío estimado hasta la próxima actualización; métricas en Tablero y glosario en Datos. Con libertad para aplicar las recomendaciones sin aprobación.

**Qué hay en la API con el token gratuito** (probado ese día para ASU→MAD): `prices_for_dates` (tarifas por fecha o mes con aerolínea, vuelo, transbordos, duración, precio, agencia y enlace), `grouped_prices` (mínimo por día o mes), `get_latest_prices` (lo más barato visto en 48 h), `get_special_offers` (ofertas por aerolínea desde el origen), `city-directions` (destinos populares con precio), `month-matrix`; `nearest-places-matrix` y `week-matrix` devolvieron 400. No incluye búsqueda en vivo (la Flight Search API se habilita a pedido a afiliados con tráfico). Para el 19/01/2027 el cache tenía una sola tarifa ASU→MAD (USD 779, 4 transbordos, 61 h): el cache se llena a medida que la gente busca.

- **El enlace trae lo que la API no da como campo** (`leerEnlace` en `packages/core/src/precios.ts`): `t=` = aerolínea + salida + llegada (hora local como epoch) + duración + cadena de aeropuertos (ASUGRULISMAD); `search_date` = cuándo se vio la tarifa; `static_fare_key=TY|P1|H1|L0|CH0|R0|TBC0`. Se probó sobre las 1.590 tarifas del dataset anterior: 100 % parsean. Con el itinerario cada tarifa es un vuelo concreto (clave: par, día, aerolínea, itinerario) y con las horas se **encadenan dos boletos**.
- **Equipaje, inferido y declarado**: la clave de tarifa no está documentada. En el dataset las low cost (easyJet, Ryanair, JetSMART, Wizz, Vueling) salen `H0` y las de red (Iberia, GOL, Azul, Air Europa) `H1`; las tarifas más baratas siempre `L0`. Se lee H como mano y L como bodega, exactitud "aproximada" en Datos, "no informado" si la clave no viene. El criterio 3 del orden se aplica con eso; hoy casi todo es "sin bodega" y el criterio rara vez desempata, pero queda aplicado y visible.
- **Combinaciones** (`packages/core/src/mercado.ts`): un boleto de un origen candidato a un destino candidato (los del modelo: el pedido más sus alternativos, con km de traslado), o dos boletos donde el segundo sale del aeropuerto en que termina el primero con espera entre `mercado.conexionMinHoras` 3 y `conexionMaxHoras` 24 (sin protección de conexión: aduana, valija, check-in). Sin límite de transbordos dentro de cada boleto. Horas totales = vuelos + esperas (las horas locales del enlace son comparables en el mismo aeropuerto). Escalas = transbordos + cambios de boleto. Aerolíneas = vendedoras distintas (la API no dice quién opera cada tramo). Tres boletos no se arman: la espera y el riesgo se multiplican y el cache rara vez tiene los tres.
- **Orden**: exactamente el del dueño, lexicográfico del 1 al 6, y después la fecha. Un origen alternativo va después del pedido aunque tenga la más barata (criterio 1 manda); la cabecera de cada grupo muestra cuántas y desde cuánto, y el Tablero compara los orígenes. Tope `mercado.maxPorOrigen` 150 por aeropuerto de salida.
- **Ventana de salida**: fecha pedida ± `mercado.flexDiasDefecto` 3 (elegible: ese día / ±3 / ±7 / ±15). Razón: el cache es ralo y las tarifas bajas suelen estar en días vecinos; cada fila dice su día.
- **Corridas sin borrar** (`scripts/precios-travelpayouts.ts`): cada `pnpm precios` agrega sus tarifas con `encontradoEn` y una entrada en `corridas`; se conservan `precios.diasHistorial` 90 días. Lo vigente es la última versión de cada tarifa (`ultimos`); las anteriores miden el desvío. El dataset con formato anterior se aparta como `precios.anterior-AAAA-MM-DD.json`, no se borra. El par pedido se baja siempre, aunque el modelo no tenga boleto único.
- **Antigüedad y desvío por fila**: "vista hace N días" cuenta desde `search_date` (la más vieja de los boletos), no desde la bajada. Desvío estimado = días × tasa diaria. Tasa: la **medida** (mediana del cambio entre corridas / días entre ellas) en cuanto hay dos corridas en días distintos; hasta entonces `precios.desvioDiarioSupuestoPct` 1 % por día (supuesto: las tarifas de largo radio se mueven del orden de 5–10 % por semana; se reemplaza por la medida sola). Se muestra cuál de las dos es.
- **Cadencia recomendada** (`precios.cadencia`): diaria a menos de 14 días del viaje, cada 3 días hasta 60, semanal más lejos. Razón: el cache refleja búsquedas de ~2 días y las tarifas se mueven más cerca de la fecha; una corrida de ASU→MAD son ~8 minutos de API gratuita. Cada fila dice cuál le toca y marca "refrescar" en rojo cuando es más vieja; el Tablero dice qué porcentaje hay que refrescar. `cadenciaDias` 7 sigue como vencimiento del dataset en Datos.
- **Estructura**: Rutas es el mercado (`GET /mercado`, `Mercado.tsx`, `FilaMercado.tsx`); el modelo sin precios sigue completo, plegado abajo con su formulario (es lo que elige los pares que baja `pnpm precios` y explica por qué). Tablero: más barata / más corta / menos escalas y qué se paga por menos escalas, dónde está lo barato (salida, aerolíneas con mínimo, escalas, agencias, día), frescura (a refrescar, antigüedad, desvío, equipaje informado) y las corridas; el resumen del modelo plegado. Datos: glosario del mercado primero y tres fichas nuevas (itinerario/horarios/agencia, equipaje, antigüedad/desvío).
- **Recomendaciones aplicadas sin pedir**: encadenar boletos por hora real en vez de por fecha; contar la antigüedad desde que se vio y no desde la bajada; mostrar agencia y enlace directo a la búsqueda; ventana ±3 por defecto; el par pedido siempre en la bajada; cadencia por anticipación en vez de una sola; apartar (no borrar) el dataset viejo.
- **Límites declarados**: sólo ida; la API da la aerolínea vendedora, no la operadora por tramo; el equipaje es inferido; el precio es el que otro usuario vio (al abrir Aviasales puede diferir; el desvío es la medida de eso); pares chicos y fechas lejanas tienen poco o nada, y la fila lo dice en vez de inventar.

## Fase 16 (17/09/2026) — bajada por continentes con la prioridad del dueño; destino continente

Pedido: ASU→MAD era un ejemplo. Priorizar la información de la API en este orden: 1. orígenes de América → destinos de Europa; 2. Europa → América; 3. Europa → Asia; 4. América → Asia. Pregunta previa del dueño: "¿estas son todas las que encontraste o decidiste traer algunas?": la bajada de la Fase 14/15 dependía de los pares que proponía el modelo (73 para ASU→MAD) y de 6 meses; había criterios míos en el medio.

**Dos hallazgos de la API que cambian la estrategia** (probados con el token ese día):
- Un par **sin mes** devuelve el mínimo de cada fecha de todo el horizonte (ASU→GRU 30 fechas, EZE→MAD 86), más que pidiendo seis meses por separado (25 y 63). Un pedido por par en vez de seis.
- Un origen **sin destino** devuelve un resumen de a qué destinos hay cache (ASU: 40 destinos, 113 tarifas): sirve para descubrir pares reales sin depender del modelo. Sin origen ni destino no anda.

- **Catálogo con continente** (`AeropuertoGeo.continente`, códigos de OurAirports): NA 1.349, SA 416, EU 736, AS 1.251, AF 485, OC 332 aeropuertos grandes y medianos. "América" = NA + SA.
- **`pnpm precios [pedidos]`** (`bajada` en config): por grupo en orden de prioridad, los aeropuertos de salida con servicio regular del continente, los de más salidas (registros VRS) primero. Por origen: si no se lo preguntó en `redescubrirDias` 30, un pedido sin destino; después un pedido por par hacia los destinos del grupo y, con `hubsDelOrigen`, hacia los aeropuertos grandes del propio continente (el primer boleto de un encadenado ASU→GRU + GRU→MAD). Un par no se vuelve a pedir dentro de `precios.cadenciaDias` 7. `maxPedidosPorCorrida` 1.500 (~25 min de API gratuita, un pedido por segundo); se guarda cada 200 pedidos y **la corrida siguiente sigue donde quedó** (lo fresco se salta, el frente avanza; pasada la cadencia, lo de arriba de la prioridad se refresca antes de seguir con la cola, que es lo que se quiere). Primera corrida de prueba con 60 pedidos: ORD (Chicago) descubierto, 93 destinos, 59 pares, 723 tarifas.
- **`pnpm precios ORIGEN DESTINO`** sigue: los pares del modelo para un par, ahora sin mes (1–2 min). `mesesAdelante` y `maxPares` salen de config.
- **Dataset**: `pares` con `bajadoEn` y `grupo` (null = a mano); `descubrimientos` por origen. El formato de la Fase 15 se aparta como `precios.anterior-2026-09-17.json` (no se borra; se rehace con `pnpm precios ASU MAD`).
- **Destino continente** en Rutas: el combobox de destino ofrece "Europa — todos los aeropuertos con tarifas" (y América del Norte/Sur, Asia, África, Oceanía). Con eso la lista es "desde este aeropuerto a distintos destinos, por precio" (el criterio 1 del dueño); cada fila dice a dónde llega. Orígenes: los candidatos del modelo (pedido + alternativos con km); llegadas: todo aeropuerto del continente con tarifas. `GET /mercado?destino=EU`; `GET /mercado/cobertura` informa por grupo cuántos pares, tarifas y aeropuertos de salida recorridos, y el formulario lo muestra.
- **Lo que sigue dependiendo de un criterio, declarado**: sólo ida; `limit=1000` por pedido (los pares sin destino de hubs enormes pueden quedar cortos: se ve en `descubrimientos`); sólo aeropuertos grandes y medianos con servicio regular; América del Norte y del Sur juntas ordenadas por salidas (los hubs de EE.UU. van primero; si el dueño quiere Sudamérica antes, se parte el grupo 1 en dos en config).

## Fase 16.1 (17/09/2026) — Sudamérica antes que Norteamérica; Rusia fuera; calendario con sólo los días con tarifas

Pedidos del dueño tras la primera corrida por continentes: (1) sí a excluir Rusia; (2) Sudamérica → Europa antes que Norteamérica → Europa; (3) en el calendario, habilitar sólo las fechas para las que hay información de la API una vez elegidos origen y destino.

- **`bajada.paisesExcluidos: ["RU"]`**: ni como origen ni como destino de la bajada, ni como llegada cuando el destino es un continente. Razón: OurAirports pone a Rusia en Europa y Aviasales (ruso) concentra ahí un cuarto de las tarifas (Moscú VKO/SVO/DME y LED eran los destinos con más tarifas de la corrida 1); consumían presupuesto sin interés del dueño. ASU → Europa el 10/11 pasa de 98 a 81 combinaciones (las 17 eran vía Moscú).
- **Grupos**: 1. SA → EU, 2. NA → EU, 3. EU → NA+SA, 4. EU → AS, 5. NA+SA → AS. Los pares ya bajados guardan el grupo como **clave estable** ("NA+SA→EU") y no como número de prioridad: al reordenar config, lo bajado antes sigue contando en su grupo real (los 1.524 pares de la corrida 1 quedaron como "NA+SA→EU", que hoy no es ningún grupo: Datos y el formulario los cuentan aparte).
...
- **19.2 (19/09/2026)**: si el API no responde durante la vigilancia (servidor apagado: "Failed to fetch"), la búsqueda en vivo no aborta: avisa, espera el minuto y reintenta; lo buscado en Aviasales ya está en su cache y se trae cuando el API vuelve.

## Fase 20 (19/09/2026) — distintivo low cost y "rutas para buscar a mano"

Pedido del dueño: marcar las aerolíneas low cost en Rutas y Combinaciones, y tener el output "Combinaciones − Rutas": las rutas que existen en el grafo y no tienen precio en el mercado, para buscarlas a mano, sabiendo en cuáles actúa una low cost (barata, pero con equipaje de mano; con bodega la ventaja se pierde).

- **Distintivo**: `cobertura.aerolineasBajoCosto` lleva a la web la lista `fase6.aerolineasPerfilBajoCosto`; `Aerolinea.tsx` pone "low cost" al lado del nombre en Rutas (cada boleto y la columna 6) y en Combinaciones (vendedoras, tramo final; en las operadoras por código, `(lc)`). Una **ruta** cuenta como "con low cost" sólo si una low cost **vende** uno de sus boletos principales (`aerolineas`, `aerolineasPrevio`): las operadoras de cada tramo y las del vuelo aparte del tramo final son opciones, no lo que se compra (contarlas marcaba 6 400 de 7 300 rutas ASU→LIS porque Ryanair/easyJet vuelan a Lisboa desde casi toda Europa). Es una **lista supuesta** (Datos ya la declara así), no un dato de la tarifa: el equipaje real de cada tarifa sigue siendo el de la columna 3.
- **Lista ampliada**: de 9 a 54 códigos, sólo aerolíneas presentes en el catálogo VRS y de perfil bajo costo puro (ultra/low cost de América, Europa, Golfo y Asia: Volaris, Viva, Sky, Allegiant, Ryanair UK, easyJet Europa/Suiza, Wizz Malta/UK, Volotea, Norwegian, Norse, French Bee, Jet2, Eurowings, Transavia, Pegasus, SunExpress, Air Arabia, flydubai, Jazeera, IndiGo, SpiceJet, Air India Express, AirAsia/X, Thai AirAsia, VietJet/Thai VietJet, Cebu, Jetstar/Asia/Japón, Peach, Jeju, T'way, Jin, Air Busan, Air Seoul, Spring). Quedan fuera las híbridas (Southwest, JetBlue, WestJet, Transat, Condor, Discover) y Wingo (comparte código P5 con Copa Colombia en el catálogo). Efecto colateral aceptado: en el modelo estas aerolíneas reciben el factor de perfil de ofertas (`fase6`), que sólo pesa en qué pares baja "Actualizar este par".
